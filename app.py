...
DOCOMO_IMAP_SERVER = os.environ.get("DOCOMO_IMAP_SERVER", "imap.spmode.ne.jp")
...
def save_import_log(status, message, imported_count=0, forwarded_count=0, deleted_count=0):
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO import_logs
                    (status, message, imported_count, forwarded_count, deleted_count)
                VALUES
                    (%s, %s, %s, %s, %s);
            """, (status, message, imported_count, forwarded_count, deleted_count))
        conn.commit()
...
def extract_ticket_info(subject, body):
    ...
    for pattern in date_patterns:
        ...
        if match:
            y, m, d = match.groups()
            expire_date = f"{int(y):04d}-{int(m):02d}-{int(d):02d}"
            break
...
def get_gmail_service():
    if not GMAIL_CREDENTIALS_JSON or not GMAIL_TOKEN_JSON:
        raise Exception("GMAIL_CREDENTIALS_JSON または GMAIL_TOKEN_JSON が未設定です")
...
def send_to_gmail(service, subject, body):
    if not YUTAI_GMAIL_TO:
        raise Exception("YUTAI_GMAIL_TO が未設定です")

    message = EmailMessage()
    message["To"] = YUTAI_GMAIL_TO
    message["Subject"] = "[株主優待転送] " + subject
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                ...
                VALUES
                    (%s, %s)
                ON CONFLICT (docomo_uid) DO NOTHING;
            """, (docomo_uid, subject))
        conn.commit()


def forward_docomo_yutai_to_gmail():
    if not DOCOMO_IMAP_USER or not DOCOMO_IMAP_PASSWORD:
        raise Exception("DOCOMO_IMAP_USER または DOCOMO_IMAP_PASSWORD が未設定です")

    service = get_gmail_service()

    forwarded_count = 0

    mail = imaplib.IMAP4_SSL(DOCOMO_IMAP_SERVER, 993)
    mail.login(DOCOMO_IMAP_USER, DOCOMO_IMAP_PASSWORD)
    mail.select("INBOX")

    status, data = mail.search(None, "ALL")
    if status != "OK":
        mail.logout()
        raise Exception("ドコモメール検索に失敗しました")
...
"""
            ...
            send_to_gmail(service, subject, forward_body)
            mark_forwarded(docomo_uid, subject)
            forwarded_count += 1
        else:
            mark_forwarded(docomo_uid, subject)
    ...
    save_import_log(
        "success",
        "ドコモメール確認・優待メール転送が完了しました。ドコモ側は削除していません。",
        imported_count=0,
...
def import_yutai_from_gmail_and_trash_unnecessary():
    service = get_gmail_service()
    ...
    for item in messages:
        ...
        if is_yutai_mail(subject, body_text):
            company, balance, expire_date = extract_ticket_info(subject, body_text)
            ...
            with get_conn() as conn:
                with conn.cursor() as cur:
                    ...
                    if not exists:
                        cur.execute("""
                            INSERT INTO tickets
                                (company, balance, expire_date, category, source, gmail_message_id)
                            VALUES
                                (%s, %s, %s, %s, %s, %s);
                        """, (
                            company,
...
def add_ticket():
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO tickets
                    (company, balance, expire_date, category, source)
                VALUES
                    (%s, %s, %s, %s, %s);
            """, (company, balance, expire_date, category, "manual"))
        conn.commit()
...
def forward_docomo():
    try:
        ...
        save_import_log("error", str(e), 0, 0, 0)
...
def import_gmail():
    try:
        import_yutai_from_gmail_and_trash_unnecessary()
    except Exception as e:
        save_import_log("error", str(e), 0, 0, 0)
